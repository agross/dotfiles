...
function {
  ...
  local git_branch="%{$fg[green]%}\$(prompt_agross_scm_branch)%{$reset_color%}"
  local git_status_symbols="%{$fg_bold[red]%}\$(prompt_agross_scm_status)%{$reset_color%}"
  local jobs="%(1j.%{$fg_bold[magenta]%} %j jobs%{$reset_color%}.)"
  local git_or_exit_status="%(?.\$(prompt_agross_symbol).%{$fg_bold[red]%}%S↑%s%{$reset_color%})"
