...
alias drestart="docker ps --no-trunc --format='table {{.Image}} {{.Names}}' | command grep --extended-regexp '^sha256:[0-9a-f]{64}' | cut -d ' ' -f2 | cut -d _ -f 1 | xargs --no-run-if-empty --verbose sudo systemctl restart"
